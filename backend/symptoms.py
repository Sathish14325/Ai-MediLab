...
sym_des = pd.read_csv("./HealthPredict/symtoms_df.csv")
precautions = pd.read_csv("./HealthPredict/precautions_df.csv")
workout = pd.read_csv("./HealthPredict/workout_df.csv")
description = pd.read_csv("./HealthPredict/description.csv")
medications = pd.read_csv("./HealthPredict/medications.csv")
diets = pd.read_csv("./HealthPredict/diets.csv")
...
def helper(dis):
    ...
    return desc[0] if len(desc) > 0 else "", \
           pre[0].tolist() if len(pre) > 0 else [], \
           med.tolist() if len(med) > 0 else [], \
           die.tolist() if len(die) > 0 else [], \
           wrkout[0] if len(wrkout) > 0 else ""
...
try:
    ...
    dis_des, precaution_list, med_list, diet_list, workout_plan = helper(predicted_disease)

    response = {
        "predicted_disease": predicted_disease,
        "dis_des": dis_des,
        "my_precautions": precaution_list,
        "medications": med_list,
        "rec_diet": diet_list,
        "workout": workout_plan
    }

    print(json.dumps(response))  # ✅ only valid JSON output
